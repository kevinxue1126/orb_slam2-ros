...
System::System(const string strVocFile, const string strSettingsFile, const eSensor sensor,
               const std::string & map_file, bool load_map):
               ...
               map_file(map_file), load_map(load_map)
{
    ...
    strSettingsFile = strSettingsFile;
...
void System::LoadSystemMap(const string &fileName)
{
     SystemSetting* systemSetting = new SystemSetting(mpVocabulary);
  
     systemSetting->LoadSystemSetting(strSettingsFile);
     mpMap->LoadMap(fileName,systemSetting);
}
...
void System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp)
{
    ...
    // Check mode change
    {
        ...
        if(mbActivateLocalizationMode)
        {
            ...
            mpTracker->InformOnlyTracking(true);// When positioning, only track
...
void System::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    // Check mode change
    {
        ...
        if(mbActivateLocalizationMode)
        {
            ...
            mpTracker->InformOnlyTracking(true);// When positioning, only track
            mbActivateLocalizationMode = false;// prevent repeated execution
        }
        if(mbDeactivateLocalizationMode)
        {
            mpTracker->InformOnlyTracking(false);
            mpLocalMapper->Release();
            mbDeactivateLocalizationMode = false;// prevent repeated execution
        }
    }
...
void System::SaveSystemMap(const string &fileName)
{
    mpMap->SaveMap(fileName);
} 
